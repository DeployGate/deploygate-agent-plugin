  ...
  server.tool(
    ...
    "Upload an app binary (IPA/APK/AAB) to DeployGate. Optionally specify a distribution page to update. If both distribution_key and distribution_name are specified, distribution_key takes priority and distribution_name is ignored. When distribution_name creates a new page, it is created with active=false.",
    {
      ...
      distribution_key: z
        ...
        .optional()
        .describe(
          "Distribution page key (access_key) to update. Takes priority over distribution_name.",
        ),
      distribution_name: z
        .string()
        .max(255)
        .optional()
        .describe(
          "Distribution page name. Creates a new page if not found (with active=false). Ignored if distribution_key is also specified.",
        ),
      release_note: z
        .string()
        .optional()
