...
export function registerNotificationTools(server) {
    server.tool("get_notification_settings_url", "Generate the URL for configuring Slack/Teams/Chatwork notifications. Supports both distribution-level and app-level notification settings. The user needs to open this URL in a browser to complete the setup.", {
        level: z
            .enum(["distribution", "app"])
            .describe("Notification level: 'distribution' for a specific distribution page, 'app' for all app activity"),
        access_key: z
            .string()
            .optional()
            .describe("Distribution page access_key (required when level is 'distribution')"),
        owner_name: z
            .string()
            .optional()
        ...
        owner_type: z
            ...
            .optional()
            .describe("Owner type: 'organization' or 'user' (required when level is 'app'). Note: URL paths differ between owner types."),
        platform: z
            .enum(["ios", "android"])
            .optional()
        ...
        app_id: z
            .string()
            .optional()
            .describe("App ID / package name / bundle identifier (required when level is 'app')"),
    }, async (args) => {
        let url;
