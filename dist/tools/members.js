import { z } from "zod";
import { DeployGateApiError } from "../client.js";
export function registerMemberTools(server, client) {
    server.tool("add_member", "Add a member to a DeployGate project with the specified role. This orchestrates multiple API calls: (1) add to workspace, (2) add to project, (3) add to team. For testers, also (4) assigns the tester team to the specified app. Handles duplicates gracefully: workspace returns 400 (already_joined_member) which is skipped, project/team additions are upserts (silent success). Free plan has a 2-member limit; exceeding it returns a 403 with upgrade guidance.", {
        ...
        platform: z
            .enum(["ios", "android"])
            .optional()
            .describe("App platform (required when role is 'tester' to assign the tester team to the app)"),
        app_id: z
            .string()
            .optional()
            .describe("App ID (required when role is 'tester' to assign the tester team to the app)"),
    }, async (args) => {
        ...
        try {
            ...
            if (e instanceof DeployGateApiError &&
                e.errorType === "already_joined_member") {
                steps.push("✓ Already in workspace (skipped)");
            }
            else if (e instanceof DeployGateApiError &&
        ...
        if (args.role === "tester" && args.platform && args.app_id) {
            await client.assignTeamToApp(args.project, args.platform, args.app_id, "tester");
    ...
    server.tool("list_members", "List members of a specific team in a project.", {
        project: z.string().describe("Project (organization) name"),
        team: z
            .enum(["owner", "developer", "tester"])
    ...
    server.tool("remove_member", "Remove a member from a team. This removes the user from the specified team only; they remain in the workspace and project.", {
        project: z.string().describe("Project (organization) name"),
        team: z
            .enum(["owner", "developer", "tester"])
        ...
        const results = await client.removeTeamMember(args.project, args.team, args.user);
        return {
            content: [
                {
                    type: "text",
